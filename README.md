...
#### Beberapa Function yang Digunakan:

- **`bind()`**: Menghubungkan `TcpListener` dengan alamat dan port tertentu pada mesin local (disini saya menggunakan `127.0.0.1:7878`).

- **`unwrap()`**: Method yang digunakan untuk mengekstrak nilai dari `Result` atau `Option`. Kalau hasilnya adalah `Ok` atau `Some`, maka value yang terkandung di dalamnya akan direturn. Tapi kalau hasilnya `Err` atau `None`, maka `unwrap()` akan menyebabkan program mengalami panic dan berhenti. 

- **`incoming()`**: Return iterator yang menghasilkan koneksi yang masuk dalam bentuk `Result<TcpStream, Error>`. Setiap elemen iterator tersebut punya sebuah stream yang bisa digunakan untuk berkomunikasi dengan client.

- **`BufReader`**: Digunakan untuk membaca data dari stream baris per baris. Ini sangat berguna kalau kita menerima request dari browser yang berisi banyak header dan body.
...
- **BufReader dan Memproses Request**: Server menggunakan `BufReader` untuk membaca data dari koneksi dan mengambil baris per baris yang membentuk HTTP request.
...
1. **Memodifikasi `handle_connection`:**
    - Function `handle_connection` diubah untuk membaca file `hello.html` menggunakan function `fs::read_to_string` dan mengirimkan isinya sebagai HTTP response.
    - `fs::read_to_string("hello.html")`: Function ini digunakan untuk membaca seluruh konten file `hello.html` dan return sebagai string. Function ini memudahkan kita untuk mengambil isi file dan mengirimkannya ke browser sebagai bagian dari HTTP response.
    - Saya juga menambahkan header `Content-Length` yang memberi tahu browser berapa panjang konten HTML yang akan diterima.
...
- Kalau path yang diminta adalah `/`, server akan mengirimkan page `hello.html` dengan status `200 OK`.
- Kalau path yang diminta adalah `/bad` atau path yang tidak valid, server akan mengirimkan page error 404 dengan status `404 Not Found`.
...
- Saya juga menerapkan beberapa prinsip _clean code_ seperti, 
    - **SOLID**: Functions `get_request_path` dan `generate_response_content` membuat kode lebih mudah diperluas tanpa mengubah logic utama.
    - **DRY**: Pengulangan dalam pembuatan respons dan pengambilan path sudah dipisahkan menjadi functions terpisah.
    - **KISS**: Struktur kode tetap sederhana dan mudah dibaca dengan menggunakan function terpisah untuk setiap bagian dari proses. 
...
Pada milestone 4 ini, saya menambahkan simulasi **slow response** pada server dengan menggunakan `thread::sleep`. Function ini mensimulasikan sebuah request yang memerlukan waktu 10 seconds untuk diproses, yang bertujuan untuk menunjukkan masalah pada server yang berjalan dalam **single-threaded**.

#### Apa _insight_ yang saya dapatkan:
- **Single-threaded Server:**
   - Dengan server yang hanya bisa menangani satu request pada satu waktu, saya menyadari kalau ketika sebuah request (seperti `/sleep`) sedang diproses, server tidak bisa memproses request lain. Hal ini menyebabkan **delay** pada request yang masuk setelahnya, bahkan kalau request itu tidak membutuhkan waktu lama untuk diproses.
...
Pada bagian bonus ini, saya mengubah function `new` yang sebelumnya langsung membuat objek `ThreadPool` menjadi function `build` yang return `Result`. Perubahan ini dilakukan untuk memperbaiki penanganan kesalahan, terutama dalam kasus di mana ukuran thread yang diberikan tidak valid.

#### **Perbedaan antara `build()` dan `new()`:**

- **`new()`**: Sebelumnya, function `new` menginisialisasi objek `ThreadPool` tanpa memeriksa validitas parameter yang diberikan. Kalau ukuran thread adalah 0 atau nilai negatif, ini menyebabkan program panic, dan server akan berhenti berfungsi tanpa penjelasan lebih lanjut.

- **`build()`**: Function `build` menggantikan `new` dan mengembalikan `Result`, yang lebih fleksibel dalam menangani kesalahan. Kalau ukuran thread yang diberikan tidak valid (seperti 0 atau nilai negatif), function ini akan mengembalikan `Err` dengan pesan yang menjelaskan kalau ukuran yang diberikan tidak valid. Ini memastikan kalau aplikasi tidak akan langsung crash dan memberikan penanganan kesalahan yang lebih baik.
